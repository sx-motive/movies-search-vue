...
async function getData() {
  const res = await fetch(`${URL}?apikey=${KEY}&s=${input.value}&plot=full`);
...
<template>
  <main>
    <div class="header">
      ...
      <input placeholder="looking for..." type="text" v-model="input" />
    </div>
    <div class="wrapper">
      <div class="movie-wrap" v-for="item in data">
        <div class="img-wrap" v-if="item.Poster !== 'N/A'">
          <img :src="item.Poster" :alt="item.Title" />
        </div>
        <div class="img-wrap" v-if="item.Poster == 'N/A'">no image</div>
        <span class="title"
...
@font-face {
  font-family: 'Apercu';
  src: url('https://cdn.jsdelivr.net/gh/sx-motive/cdn@master/fonts/apercu/ApercuPro-Bold.woff')
    format('woff');
  font-weight: 700;
  font-style: normal;
}
...
@font-face {
  font-family: 'Apercu';
  src: url('https://cdn.jsdelivr.net/gh/sx-motive/cdn@master/fonts/apercu/ApercuPro-Medium.woff')
    format('woff');
  font-weight: 500;
  font-style: normal;
}
...
input {
  ...
  border: none;
  outline: rgb(54, 54, 54) solid 0.5px;
